...
@[simp] abbrev loopCoreFrame : UInt32 := 1048432
@[simp] abbrev loopPairOut : UInt32 := 1048440
@[simp] abbrev loopError : UInt32 := 1048464
@[simp] abbrev loopIterator : UInt32 := 1048504

def decodeLoopPairBaseStore (store : MachineStore Universal.State)
    (inputPtr len chunkIndex : UInt32) : MachineStore Universal.State :=
  let mem1 := store.wasm.mem.write32 (loopIterator + 4) (len - 2)
  let mem2 := mem1.write32 loopIterator (2 + inputPtr)
  { store with wasm := { store.wasm with
      mem := mem2.write32 (loopIterator + 12) (1 + chunkIndex) } }

def decodeLoopPairValidStore (store : MachineStore Universal.State)
    (inputPtr len chunkIndex : UInt32) (byte : UInt8) :
    MachineStore Universal.State :=
  let base := decodeLoopPairBaseStore store inputPtr len chunkIndex
  let mem1 := base.wasm.mem.write8 (loopPairOut + 1) byte
  { base with wasm := { base.wasm with mem := mem1.write8 loopPairOut 1 } }

def decodeLoopPairInvalidStore (store : MachineStore Universal.State)
    (inputPtr len chunkIndex : UInt32) (bad : UInt8) (index : UInt32) :
    MachineStore Universal.State :=
  let mem1 := store.wasm.mem.write32 (loopIterator + 4) (len - 2)
  let mem2 := mem1.write32 loopIterator (2 + inputPtr)
  let mem3 := mem2.write32 loopError (bad.toUInt32 &&& 255)
  let mem4 := mem3.write32 (loopError + 4) index
  let mem5 := mem4.write32 (loopIterator + 12) (1 + chunkIndex)
  let mem6 := mem5.write8 (loopPairOut + 1) bad
  { store with wasm := { store.wasm with mem := mem6.write8 loopPairOut 0 } }

def decodeLoopPairEmptyStore (store : MachineStore Universal.State) :
    MachineStore Universal.State :=
  let mem1 := store.wasm.mem.write8 (loopPairOut + 1) 0
  { store with wasm := { store.wasm with mem := mem1.write8 loopPairOut 0 } }

def loopPairPreparedStore (store : MachineStore Universal.State)
    (inputPtr len chunkIndex : UInt32) (hi lo oldTag oldPayload : UInt8) :
    MachineStore Universal.State :=
  let mem0 := store.wasm.mem.write32 (loopIterator + 4) len
  let mem1 := mem0.write32 (loopIterator + 16) loopError
  let mem2 := mem1.write32 (loopIterator + 8) 2
  let mem3 := mem2.write32 loopIterator inputPtr
  let mem4 := mem3.write32 (loopIterator + 12) chunkIndex
  let mem5 := mem4.write32 loopError 1114114
  let mem6 := mem5.write32 (loopError + 4) 0
  let mem7 := mem6.write8 inputPtr hi
  let mem8 := mem7.write8 (inputPtr + 1) lo
  let mem9 := mem8.write8 loopPairOut oldTag
  { store with wasm := { store.wasm with
      mem := mem9.write8 (loopPairOut + 1) oldPayload } }

def loopPairStandaloneConfig (store : MachineStore Universal.State) :
    Config Universal.State :=
  ⟨.running ⟨⟨[], [], [.i32 loopIterator, .i32 loopPairOut]⟩,
    [.call 3], 0, [], [], []⟩, store⟩

def loopPairStandaloneReturn (store : MachineStore Universal.State) :
    Config Universal.State :=
  ⟨.running ⟨⟨[], [], []⟩, [], 0, [], [], []⟩, store⟩

def loopKeep32 (value : UInt32) : UInt32 := value

theorem loop_core_addresses :
    loopPairOut + 1 = 1048441 ∧ loopIterator + 4 = 1048508 ∧
    loopIterator + 8 = 1048512 ∧ loopIterator + 12 = 1048516 ∧
    loopIterator + 16 = 1048520 ∧ loopError + 4 = 1048468 := by
  decide

set_option maxRecDepth 100000 in
theorem decodeLoopPair_empty_reaches
    ...
    Reaches
      ...
      ⟨.running ⟨⟨callerParams, callerLocalValues, stack⟩,
        ...
        decodeLoopPairEmptyStore store⟩ := by
  ...
  simp [decodeLoopPairEmptyStore]
  exact ⟨[], .refl _⟩

set_option maxRecDepth 100000 in
set_option maxHeartbeats 4000000 in
theorem decodeLoopPair_valid_reaches
    (store : MachineStore Universal.State)
    ...
    (herrorRead : store.wasm.mem.read32 (loopIterator + 16) = errorPtr)
    ...
    Reaches
      ...
      ⟨.running ⟨⟨callerParams, callerLocalValues, stack⟩,
        code, arity, remainder, controls, calls⟩,
        decodeLoopPairValidStore store inputPtr len chunkIndex
          ((loRoute.nibble lo |||
  ...
  all_goals
    ...
    have hinput1Mod' :
      ...
      simpa only [inputPtr, loopIterator, UInt32.toNat_ofNat] using hinput1Mod
    simp only [HexRoute.valid, Mem.read8, inputPtr, loopIterator] at hlo
    ...
    all_goals
      ...
      dsimp only
      apply Reaches.prepend (Step.returnFromCallFallthrough (by simp))
      simp [decodeLoopPairValidStore, decodeLoopPairBaseStore,
