...
theorem decodePair_invalid_high_reaches
    ...
    Reaches
      ...
      ⟨.running ⟨⟨callerParams, callerLocalValues, stack⟩,
        ...
        decodePairInvalidStore store inputPtr len chunkIndex hi
  ...
  apply Reaches.prepend (Step.ltU rfl)
  change ¬ (4294967231 +
    (store.wasm.mem.bytes (store.wasm.mem.read32 1048472).toNat).toUInt32 &&&
      255 < 6) at hhiUpper
  change ¬ (4294967199 +
    (store.wasm.mem.bytes (store.wasm.mem.read32 1048472).toNat).toUInt32 &&&
      255 < 6) at hhiLower
  ...
  simp [decodePairInvalidStore, decodePairBaseStore, Mem.write32]
  exact ⟨[], .refl _⟩
