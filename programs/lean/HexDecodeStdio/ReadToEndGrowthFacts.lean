...
theorem ByteGrowSuccess.preserves_read32_before_metadata
    {input consumed remaining : List UInt8}
    {store allocStore : MachineStore Universal.State}
    {capacity data length bump addr : UInt32}
    (hinv : ReadToEndInv input consumed remaining store capacity data length bump)
    (h : ByteGrowSuccess store capacity data
      (readToEndNewCapacity capacity) bump allocStore)
  ...
  have hcapacityNe : capacity ≠ 0 := by
    ...
    have := hinv.capacity_pos
    simp [hz] at this
  have hbefore : addr.toNat + 4 ≤ bump.toNat := by
    exact le_trans haddr (le_trans (by decide) (le_trans hinv.data_lower (by
      have := hinv.data_capacity_bump
      have hc := hinv.capacity_pos
      omega)))
  cases h with
...
theorem ByteGrowSuccess.grown_invariant
  ...
  have hstatusCapacity := h.preserves_read32_before_metadata hinv (by decide :
    decodeStatusVector.toNat + 4 ≤ 1053960)
  have hstatusPointer := h.preserves_read32_before_metadata hinv (by decide :
    (decodeStatusVector + 4).toNat + 4 ≤ 1053960)
  have hstatusLength := h.preserves_read32_before_metadata hinv (by decide :
    (decodeStatusVector + 8).toNat + 4 ≤ 1053960)
  refine
    { split := hinv.split
      input_eq := ?_
      output_eq := ?_
  ...
  · simp only [readToEndGrownStore, readToEndGrowFinishedStore,
    ...
    · exact hstatusCapacity.trans hinv.status_capacity
    all_goals decide
  · simp only [readToEndGrownStore, readToEndGrowFinishedStore,
      growResultOkStore]
    rw [Mem.read32_write32_disjoint, Mem.read32_write32_disjoint,
      Mem.read32_write32_disjoint, Mem.read32_write32_disjoint,
      Mem.read32_write32_disjoint]
    · exact hstatusPointer.trans hinv.status_pointer
    all_goals decide
  · simp only [readToEndGrownStore, readToEndGrowFinishedStore,
      growResultOkStore]
    rw [Mem.read32_write32_disjoint, Mem.read32_write32_disjoint,
      Mem.read32_write32_disjoint, Mem.read32_write32_disjoint,
      Mem.read32_write32_disjoint]
    · exact hstatusLength.trans hinv.status_length
    all_goals decide
  · simp only [readToEndGrownStore, readToEndGrowFinishedStore,
      growResultOkStore]
