...
def ReadToEndSuccess (input : List UInt8)
  ...
  ∃ store capacity data bump,
    config = decodeAfterReadConfig store ∧
    ...
    17 ≤ store.wasm.mem.pages ∧
    store.wasm.mem.pages ≤ 65536 ∧
    globalAt? store 0 = some (.i32 decodeStack) ∧
    store.wasm.mem.read32 decodeStatusVector = 0 ∧
    store.wasm.mem.read32 (decodeStatusVector + 4) = 1 ∧
    store.wasm.mem.read32 (decodeStatusVector + 8) = 0 ∧
    store.wasm.mem.read32 decodeInputVector = capacity ∧
    store.wasm.mem.read32 (decodeInputVector + 4) = data ∧
    store.wasm.mem.read32 (decodeInputVector + 8) =
    ...
    (input = [] ∨ data.toNat + capacity.toNat = bump.toNat) ∧
    (input = [] ∨ 1054000 ≤ data.toNat) ∧
    (bump = 0 ∨ 1054000 ≤ bump.toNat) ∧
    bump.toNat < 2 ^ 31 ∧
