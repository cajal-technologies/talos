...
namespace Project.HexEncodeStdio.EncodeLoop
...
abbrev loopSaved {input : List UInt8} (state : EncodeLoopState input) : UInt32 :=
  ...
  | true => Project.HexEncodeStdio.Iterator.sentinel
...
theorem func6_ascii_store {hlc : HasLC} {α : Type}
  ...
  iapply wp_localGet rfl
  inext
  iapply wp_load32 capacity p4 p5 p6 p7 $$ Hcap
  inext
  iintro Hcap
  iapply wp_localGet rfl
  inext
  iapply wp_sub
  inext
  iapply wp_leU (result := 1) (by simp [hroom])
  inext
  iapply wp_brIf (by decide) rfl
  inext
  iapply wp_localGet rfl
  inext
  iapply wp_load32 output p8 p9 p10 p11 $$ HoutputPtr
  inext
  iintro HoutputPtr
  iapply wp_localGet rfl
  inext
  ...
  iapply wp_localGet rfl
  inext
  iapply Project.HexEncodeStdio.Helpers.wp_store8_zero actual $$ Hbyte
  inext
  iintro Hbyte
  ...
  iapply wp_localTee rfl
  inext
  iapply wp_store32 (UInt32.ofNat i) p12 p13 p14 p15 $$ Hlength
  inext
  iintro Hlength
  iapply wp_localGet rfl
  inext
...
theorem func6_encode_loop_step_even {hlc : HasLC} {α : Type}
    ...
    (hroom : (1 : UInt32) ≤ capacity - position)
    (hstack : stackPtr.toNat + 20 < UInt32.size)
    (hsaved : saved ≠ Project.HexEncodeStdio.Iterator.sentinel)
    {arity : Nat} {remainder : List Value}
    ...
    runtimeModuleOwn ⟨0⟩ Project.HexStdio.«module» ∗
      ...
      (runtimeModuleOwn ⟨0⟩ Project.HexStdio.«module» -∗
        ...
        pointsTo_u32 0 (stackPtr + 8) output -∗
        pointsTo_u32 0 (stackPtr + 12) (position + 1) -∗
        pointsTo_u32 0 (stackPtr + 16) Project.HexEncodeStdio.Iterator.sentinel -∗
        pointsToBytes 0 output (out.set i digit) -∗
        ▷ WP (.running
  ...
  iintro ⟨Hruntime, Hcap, HoutputPtr, Hlength, Hiter, Hout, Hnext⟩
  ...
  simp only [encodeLoopCallTail, encodeLoopBody, encodeOuterBody,
    ...
    List.getElem?_cons_succ, List.drop_succ_cons, List.drop_zero]
  iapply Project.HexEncodeStdio.Iterator.wp_call_func18_low (stackPtr + 16) saved hsaved
    (by
  ...
  iapply wp_const
  inext
  iapply wp_ne (result := 1) (by simpa [Project.HexEncodeStdio.Iterator.sentinel] using hsaved)
  inext
  iapply wp_brIf (by decide) rfl
  ...
  iapply Hnext $$ Hruntime Hcap HoutputPtr Hlength Hiter Hout
...
theorem func6_encode_loop_step_odd_more {hlc : HasLC} {α : Type}
  ...
  simp only [encodeLoopCallTail, encodeLoopBody, encodeOuterBody,
    Project.HexStdio.func6, List.getElem?_cons_zero,
    List.getElem?_cons_succ, List.drop_succ_cons, List.drop_zero]
  iapply Project.HexEncodeStdio.Iterator.wp_call_func18_high_at (stackPtr + 16) source
...
theorem func6_encode_loop_step_odd_end {hlc : HasLC} {α : Type}
    ...
    runtimeModuleOwn ⟨0⟩ Project.HexStdio.«module» ∗
      ...
      (runtimeModuleOwn ⟨0⟩ Project.HexStdio.«module» -∗
        ...
        pointsToBytes 0 output (out.set outIndex digit) -∗
        WP (.running
          ⟨encodeLocals result (position + 1) Project.HexEncodeStdio.Iterator.sentinel
              stackPtr 1 1 (output + position) tmp1 tmp2,
  ...
  simp only [encodeLoopCallTail, encodeLoopBody, encodeOuterBody,
    Project.HexStdio.func6, List.getElem?_cons_zero,
    List.getElem?_cons_succ, List.drop_succ_cons, List.drop_zero]
  iapply Project.HexEncodeStdio.Iterator.wp_call_func18_end (stackPtr + 16) finish
    (by
      rw [show (16 : UInt32) = UInt32.ofNat 16 by rfl,
        UInt32.add_ofNat_toNat_noWrap stackPtr 16 (by decide) (by
          have hlt : stackPtr.toNat + 16 < stackPtr.toNat + 32 := by omega
...
theorem func6_encode_loop {hlc : HasLC} {α : Type}
  ...
  let StateInv : Locals → IProp (WasmHeapGF α) := fun locals => iprop%
    ∃ state : EncodeLoopState input,
      ⌜locals = loopLocals result stackPtr output state⌝ ∗
  ...
  iapply wp_loop_löb_family
    (ι := Locals)
    (locals := fun locals => locals)
    (I := StateInv)
    (initial := loopLocals result stackPtr output initial)
    (body := encodeLoopBody)
    (code := afterLoop)
    (belowStack := [])
    rfl
  intro actualLocals
  simp only [StateInv, Wasm.SmallStep.loopBodyExpr, id_eq]
  iintro Hrec Hstate
  icases Hstate with ⟨%state, %hlocals, Hinv⟩
  subst actualLocals
  icases Hinv with ⟨Hruntime, Hcap, HoutputPtr, Hlength, Hcurrent, Hcursor,
  ...
  | false =>
      ...
      isplitl [Hout]
      · iexact Hout
      iintro Hruntime Hcap HoutputPtr Hlength Hcurrent Hout
      inext
      ispecialize Hrec $$ %(encodeLocals result
        (UInt32.ofNat (2 * byteIndex) + 1)
        (Project.HexStdio.Spec.hexDigit
          (input[byteIndex].toNat % 16)).toUInt32
        stackPtr 1 1 (output + UInt32.ofNat (2 * byteIndex)) 0 0)
      iapply Hrec
      iexists next
      isplitr
      · ipureintro
  ...
  | true =>
      ...
      · let next : EncodeLoopState input :=
        ...
        ispecialize Hrec $$ %(encodeLocals result
          (UInt32.ofNat (2 * byteIndex + 1) + 1)
          (Project.HexStdio.Spec.hexDigit
            (input[byteIndex + 1].toNat / 16)).toUInt32
          stackPtr 1 1 (output + UInt32.ofNat (2 * byteIndex + 1)) 0 0)
...
end Project.HexEncodeStdio.EncodeLoop
