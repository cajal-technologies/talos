...
/-!
# Proof of the hash map reallocator

This module proves local `func58` (absolute index 61) against `Func58Spec`.
The body repeats the bump allocation sequence, copies the shorter prefix of
the old and new sizes, and retires the old allocation.  The proof is a port
of `Project.Mergesort.Func8Proof` with the hash map constants.
-/

namespace Project.RustHashMap.Func58Proof
...
private abbrev func58CopyBody : Program :=
...
private abbrev func58ArithmeticPrefix : Program :=
...
private abbrev func58GrowthBody : Program :=
...
private abbrev func58PostArithmetic : Program :=
  [.block 0 0 func58GrowthBody,
    .const 0, .localGet 4, .store32 allocatorCursor,
    .block 0 0 func58CopyBody, .localGet 2, .ret]

private theorem func58_shape :
    Project.RustHashMap.func58 =
...
private theorem twp_func58_copy_and_return
    ...
    iprop(
      ...
      (∀ finalBytes : List UInt8,
        ...
        ResumeWP [.i32 newPtr] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func58Locals oldPtr oldSize newPtr newSize temp requiredPages currentPages,
          [.block 0 0 func58CopyBody, .localGet 2, .ret], 1, [],
          currentControls,
  ...
  iapply twp_localTee
      (locals' := func58Locals oldPtr oldSize newPtr newSize oldSize requiredPages
        currentPages
        [.i32 oldSize]) (by rfl)
  simp only [func58Locals]
  ...
  ihave HoldSlice : Slices.ByteSlice 0 oldPtr oldBytes $$
      [HoldBytes]
  ...
  ihave HnewPrefix : Slices.ByteSlice 0 newPtr oldBytes $$
      [HnewPrefixBytes]
  · unfold Slices.ByteSlice
    iframe_pureexact (by simpa only [hprefixLength, holdLength] using hnewPrefixNowrap)
  ihave HnewSlice : Slices.ByteSlice 0 newPtr finalBytes $$
      [HnewPrefix HnewTail]
  · dsimp only [finalBytes]
    iapply (Slices.ByteSlice_append 0 newPtr oldBytes
...
private theorem twp_func58_commit_copy_and_return
    [WasmSmallStepGS hlc Universal.State]
    ...
    iprop(
      ...
      Slices.ByteSlice 0 newPtr newBytes ∗
      LiveBlock heapId oldId oldPtr oldLayout oldBytes ∗
      Streams input output raised ∗
      (∀ finalBytes : List UInt8,
        RuntimeContext -∗
        ...
        ResumeWP [.i32 newPtr] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func58Locals oldPtr oldSize newPtr newSize finish requiredPages
            currentPages [.i32 0],
  ...
  imod BumpHeap_commit heapId frontier history newPtr finish newLayout newBytes
      ...
      ⟨Hbump, HnewBlock⟩
  · iframe
  iapply twp_func58_copy_and_return oldPtr oldSize newPtr newSize oldLayout
...
private theorem twp_func58_claim_commit_copy_and_return
    [WasmSmallStepGS hlc Universal.State]
    (oldPtr oldSize newPtr newSize finish requiredPages currentPages
      storedCursor : UInt32)
    (oldLayout newLayout : AllocLayout)
  ...
  · iframe
  iapply Project.RustHashMap.Allocator.twp_const_alloc_freshRange_owned
  ...
  iapply twp_func58_commit_copy_and_return oldPtr oldSize newPtr newSize finish
...
private theorem twp_func58_oom
    [WasmSmallStepGS hlc Universal.State]
    (oldPtr oldSize newPtr newSize : UInt32)
  ...
  have Hoom := Project.RustHashMap.ImportProofs.func56_correct (hlc := hlc)
      (input := input) (output := output) (raised := raised)
      (callerLocals := func58Locals oldPtr oldSize newPtr newSize temp
      ...
      (s := s) (E := E) (Φ := Φ)
  unfold Func56Spec CallContract callExpr at Hoom
  simp only [List.nil_append, func58Locals] at Hoom ⊢
...
theorem func58_correct [WasmSmallStepGS hlc Universal.State] :
    Func58Spec (hlc := hlc) := by
  unfold Func58Spec CallContract callExpr
  ...
  wasm_twp_rebind Wasm.SmallStep.twp_call Project.RustHashMap.«module» 61
      Project.RustHashMap.func58Def (by decide) func58_index with Hmodule
  simp [Project.RustHashMap.func58Def, func58_shape,
  ...
  | success newPtr finish =>
      ...
      · rw [if_pos hcapacity]
        ...
        iapply twp_func58_claim_commit_copy_and_return oldPtr oldSize newPtr
      ...
      · rw [if_neg hcapacity]
        ...
        ihave HgrowFrame : iprop(
            ...
            [Henv Hcursor Hfrontier Hauth Hretired HoldPages Hblock Hstreams
              Hcont]
        · iframe
        iapply twp_memoryGrow_tracked Project.RustHashMap.«module» ⟨0⟩ pages
            (fun actualPages hmeasured => by
              ...
              iapply twp_func58_oom oldPtr oldSize newPtr newSize heapId oldId
            ...
            (fun oldPages previousPages newPages hfacts hmeasured => by
              ...
              · iapply twp_eq (result := 0) (by simp [hsentinel])
                ...
                iclose_map_runtime Hruntime with Hmodule Henv
                ihave Hnormal := BI.and_elim_l $$ Hcont
                iapply twp_func58_claim_commit_copy_and_return oldPtr oldSize
...
end Project.RustHashMap.Func58Proof
