...
private structure EntryHeapSeal where
  ...
  heap_eq : heap = insertFreshBytes entryStackHeap allocatorCursor entryCursorBytes

private opaque entryHeapSeal : EntryHeapSeal := ⟨_, rfl⟩

/-- The initial owned heap: the whole shadow stack and the allocator cursor
word. -/
def entryHeap : WasmHeapMap (Option UInt8) := entryHeapSeal.heap

theorem entryHeap_eq :
    entryHeap = insertFreshBytes entryStackHeap allocatorCursor entryCursorBytes :=
...
theorem initialResources [WasmSmallStepGS hlc Universal.State]
  ...
  ihave ⟨HcursorBytes, Hstack⟩ := entryHeap_split $$ Hheap
...
theorem twp_entry_of_spec
  ...
  have hcall := hspec heapId input entryStackBytes
