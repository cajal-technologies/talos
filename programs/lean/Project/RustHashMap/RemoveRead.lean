...
/-!
# The read phase of `map_remove`

The `map_remove` driver is local `func6`, absolute function 9.  It takes a
frame of 320 bytes below the entry stack top.  This file proves the frame
setup, the first chunk read, the read loop, and the reload of the vector
registers, up to the exit of the read block.  The frame base is concrete,
so every address fact is decidable.

This file is the instance of the read-loop core at `removeMap`, the frame map
of absolute function 9.  The driver declares six locals, so the core gets
a local tail of one slot.  That slot is an `i64`, which is the only
difference from the `map_get` driver.  The read phase never writes the
slot, so it holds zero throughout.

The frame has three regions.  The head holds 52 bytes.  The vector header
holds 12 bytes at offset 52.  The 256-byte chunk buffer starts at offset
64 and ends at the top of the frame.  This differs from the `map_len`
...
namespace Project.RustHashMap.RemoveRead
...
theorem ByteSlice_frame320 [WasmHeapGS Universal.State]
    (ptr : UInt32) (bytes : List UInt8) (hlen : bytes.length = 320) :
    Slices.ByteSlice 0 ptr bytes ⊢
      iprop(∃ head header chunk : List UInt8,
        ⌜head.length = 52 ∧ header.length = 12 ∧ chunk.length = 256⌝ ∗
        Slices.ByteSlice 0 ptr head ∗
        Slices.ByteSlice 0 (ptr + 52) header ∗
        Slices.ByteSlice 0 (ptr + 64) chunk) := by
  iintro Hbytes
  icases (ByteSlice_split_at ptr 52 bytes (by simp [hlen])).mp $$ Hbytes
    with ⟨Hhead, Hrest⟩
  isimp only [UInt32.reduceToNat] at Hhead
  isimp only [UInt32.reduceToNat] at Hrest
  icases (ByteSlice_split_at (ptr + 52) 12 (bytes.drop 52)
  ...
  iexists (bytes.take 52), ((bytes.drop 52).take 12),
    ((bytes.drop 52).drop 12)
  isplitl_pureexact (by simp [hlen])
  iframe

/-! ## The frame of `map_remove` -/

/-- The frame base of the `map_remove` driver: 320 bytes below the entry
stack top. -/
def func6Base : UInt32 := entryStackTop - 320

/-- The code of the `map_remove` driver after the read phase.  The frame
setup is 19 instructions and the read block is the 20th. -/
def func6AfterRead : Program := Project.RustHashMap.func6.drop 20

theorem func6_shape :
    Project.RustHashMap.func6 =
      readPrologue removeMap ++
        .block 0 0 (readPhaseBody removeMap) :: func6AfterRead := by
  rfl
...
driver declares six locals, and the read phase leaves local 5 at zero. -/
def afterReadLocalsRm (length capacity ptr aux4 : UInt32) : Locals :=
  ⟨[], [.i32 func6Base, .i32 length, .i32 capacity, .i32 ptr,
    .i32 aux4, .i64 0], []⟩

/-- The resources of the driver after the read phase: the whole input is
in the vector, and the input stream is empty. -/
def AfterReadRm [WasmSmallStepGS hlc Universal.State]
  ...
  iprop(
    RuntimeContext ∗
    StackPointer func6Base ∗
    StackReserve (func6Base - 16) reserve ∗
    Slices.ByteSlice 0 func6Base head ∗
    VecU8 heapId (func6Base + 52) capacity ptr input ∗
    Slices.ByteSlice 0 (func6Base + 64) chunk ∗
    BumpHeap heapId storedCursor frontier history ∗
    Streams [] output false ∗
    ⌜head.length = 52 ∧ chunk.length = 256 ∧
      PushVecFacts capacity ptr frontier⌝)

/-- The control frame of the read block. -/
def phaseFrameRm (afterRead : Program) : ControlFrame :=
  { kind := .block, paramArity := 0, resultArity := 0,
    body := .block 0 0 (firstReadBody removeMap) :: .const 0 ::
      .localSet 1 :: .loop 0 0 (readLoopBody removeMap) :: reloadVec removeMap,
    continuation := afterRead, belowStack := [] }

private theorem func6_initialLocals :
    Project.RustHashMap.func6Def.toLocals [] =
      ⟨[], [.i32 0, .i32 0, .i32 0, .i32 0, .i32 0, .i64 0], []⟩ := by
  rfl
...
/-- The read phase of the `map_remove` driver: the frame setup, the reads,
and the register reload.  The caller owns the 336 bytes from the reserve
base to the entry stack top. -/
theorem twp_remove_read_phase [WasmSmallStepGS hlc Universal.State]
    ...
    (hframe : frameBytes.length = 336) :
    iprop(
      RuntimeContext ∗
      StackPointer entryStackTop ∗
      Slices.ByteSlice 0 (func6Base - 16) frameBytes ∗
      BumpHeap heapId storedCursor frontier history ∗
  ...
  wasm_twp_rebind twp_store32 (address := func6Base)
    (offset := removeMap.vecOff + 8) oldLength (by decide) (by decide)
    (by decide) (by decide) with HoldLength
  wasm_twp_pures [twp_localGet]
  iapply twp_pureStep _ _ _ (fun _ => Step.constI64)
  wasm_twp_bind twp_store64 (address := func6Base)
    (offset := removeMap.vecOff)
    (oldCapacity.toUInt64 ||| (oldPtr.toUInt64 <<< 32))
    (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide) (by decide) with HoldPair => Hpair
  wasm_twp_pures [twp_localGet twp_const twp_add]
  rw [show removeMap.chunkOff + func6Base = func6Base + removeMap.chunkOff
    by decide]
  ...
  ihave Hchunk : Slices.ByteSlice 0 (func6Base + removeMap.chunkOff)
  ...
  ihave HpairWords : iprop(
      pointsTo_u32 0 (func6Base + removeMap.vecOff) 0 ∗
      pointsTo_u32 0 (func6Base + removeMap.vecOff + 4) 1) $$ [Hpair]
  · iapply (pointsTo_u32_pair_as_u64 (func6Base + removeMap.vecOff) 0 1).mpr
  ...
  ihave HoldLength := pointsTo_u32_address_eq
    (vec_addr removeMap func6Base 8).symm $$ HoldLength
  ihave Hvec : VecU8 heapId (func6Base + removeMap.vecOff) 0 1 [] $$
  ...
  iapply twp_read_chunk removeMap func6Base (List.replicate 256 0)
    input output []
    [.i32 func6Base, .i32 0, .i32 0, .i32 0, .i32 0, .i64 0]
  ...
  · -- an empty input: leave the read block at once
    ...
    isimp only [afterReadLocalsRm] at Hcont
    ihave Hnormal := BI.and_elim_l $$ Hcont
    iapply Hnormal $$ %0 %1 %storedCursor %frontier %history
      %(frameBytes.take 16) %head %(List.replicate 256 0) %0
    isimp only [show func6Base + removeMap.chunkOff = func6Base + 64
      by decide] at Hchunk
    isimp only [show func6Base + removeMap.vecOff = func6Base + 52
      by decide] at Hvec
    unfold AfterReadRm
  ...
  · -- a nonempty input: run the loop
    ...
    have Hloop := twp_read_loop hfunc98 removeMap func6Base heapId
      input output [.i64 0] (reloadVec removeMap) arity remainder
      (phaseFrameRm afterRead :: controls) calls s E Φ (by decide)
      removeMap_wf initial
    simp only [loopLocals, initial, htakeLen, phaseFrameRm, firstReadBody,
    ...
    · iintro %finalCapacity %finalPtr %finalStoredCursor %finalFrontier
      ...
      wasm_twp_pures [twp_localGet]
      wasm_twp_rebind twp_load32 (address := func6Base)
        (offset := removeMap.vecOff + 8)
      ...
      ihave Hvec : VecU8 heapId (func6Base + removeMap.vecOff)
...
end Project.RustHashMap.RemoveRead
