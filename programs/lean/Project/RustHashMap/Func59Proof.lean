...
/-!
# Proof of the hash map zeroed allocator

This module proves local `func59` (absolute index 62) against `Func59Spec`.
The body repeats the bump allocation sequence and fills the committed block
with zero bytes before it returns.  The proof is a port of
`Project.Mergesort.Func9Proof` with the hash map constants.
-/

namespace Project.RustHashMap.Func59Proof
...
private abbrev func59GrowthBody : Program :=
...
private abbrev func59PostArithmetic : Program :=
  [.block 0 0 func59GrowthBody,
    .const 0, .localGet 2, .store32 allocatorCursor,
    .block 0 0 func59ZeroBody, .localGet 1, .ret]

private abbrev func59Locals
...
private theorem twp_func59_zero_and_return
    ...
    iprop(
      ...
      (RuntimeContext -∗
        ...
        ResumeWP [.i32 base] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func59Locals size base finish requiredPages currentPages,
          [.block 0 0 func59ZeroBody, .localGet 1, .ret], 1, [],
          functionControls,
  ...
  wasm_twp_pures [twp_block] using [func59ZeroBody, func59Locals]
  ...
  ihave Hslice : Slices.ByteSlice 0 base
...
private theorem twp_func59_commit_zero_and_return
    [WasmSmallStepGS hlc Universal.State]
    ...
    iprop(
      ...
      (RuntimeContext -∗
        ...
        ResumeWP [.i32 base] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func59Locals size base finish requiredPages currentPages [.i32 0],
  ...
  imod BumpHeap_commit heapId frontier history base finish layout bytes
      ...
      ⟨Hbump, Hblock⟩
  · iframe
  iapply twp_func59_zero_and_return size base finish requiredPages currentPages
...
private theorem twp_func59_claim_commit_zero_and_return
    [WasmSmallStepGS hlc Universal.State]
    (size base finish requiredPages currentPages storedCursor : UInt32)
    (layout : AllocLayout) (heapId : GName)
  ...
  iapply twp_func59_commit_zero_and_return size base finish requiredPages
...
private theorem twp_func59_oom
    [WasmSmallStepGS hlc Universal.State]
    (size base finish requiredPages currentPages : UInt32)
  ...
  have Hoom := Project.RustHashMap.ImportProofs.func56_correct (hlc := hlc)
      ...
      (callerLocals := func59Locals size base finish requiredPages currentPages)
...
theorem func59_correct [WasmSmallStepGS hlc Universal.State] :
    Func59Spec (hlc := hlc) := by
  unfold Func59Spec CallContract callExpr
  ...
  wasm_twp_rebind Wasm.SmallStep.twp_call Project.RustHashMap.«module» 62
      Project.RustHashMap.func59Def (by decide) func59_index with Hmodule
  simp [Project.RustHashMap.func59Def, Project.RustHashMap.func59,
  ...
  ihave HcursorAt : pointsTo_u32 0 ((0 : UInt32) + 1049496)
      storedCursor $$ [Hcursor]
  · irw_exact [show (0 : UInt32) + 1049496 = allocatorCursor by decide] with Hcursor
  wasm_twp_bind twp_load32 (address := 0) (offset := 1049496) storedCursor
  ...
  · have hfinishSigned : finishNat < 2147483648 := by omega
    ...
    simp only [show Project.RustHashMap.«module».memIs64 = false by rfl,
    ...
    · iapply twp_leU (result := 1) (by rw [if_pos hfits])
      ...
      have Hclaim := twp_func59_claim_commit_zero_and_return size base finish
          (allocatorRequiredPages finish) pages.toUInt32 storedCursor layout
          heapId frontier pages history input output raised
          [{ kind := .block
             paramArity := 0
             resultArity := 0
             body := func59ArithmeticPrefix ++ func59PostArithmetic
             continuation := [.call 59, .unreachable]
             belowStack := [] }]
      ...
      simp only [func59Locals, func59ZeroBody, func59ArithmeticPrefix,
        func59PostArithmetic, func59GrowthBody, allocatorCursor, heapBase,
        Nat.toUInt32, List.cons_append, List.nil_append] at Hclaim
      ihave HcursorRaw : pointsTo_u32 0 (1049496 : UInt32) storedCursor $$
          [Hcursor]
    ...
    · iapply twp_leU (result := 0) (by rw [if_neg hfits])
      ...
      iapply twp_memoryGrow_tracked Project.RustHashMap.«module» ⟨0⟩ pages
          (fun actualPages hmeasured => by
            ...
            iapply twp_func59_oom size base finish
          ...
          (fun oldPages previousPages newPages hfacts hmeasured => by
            ...
            · iapply twp_eq (result := 0) (by simp [hsentinel])
              ...
              ihave Hnormal := BI.and_elim_l $$ Hcont
              iclose_map_runtime Hruntime with Hmodule Henv
              have Hclaim := twp_func59_claim_commit_zero_and_return size base
                  finish
