...
/-- The address of a shallower region inside a deeper one.  Copy of
`Project.RustHashMap.Func4Proof.stack_split_addr` with a general
shallow depth, because the sort depth of the remove tail is not a
numeral. -/
...
/-- Five cells back as a twenty-byte slot.  The accept arm reads four of
the five words and then hands the same bytes to `collect_entries`, so the
proof needs the direction that
...
words exposed, because the remove tail reads the twelve bytes that stay
at the source after the move. -/
...
/-- Three cells back as a twelve-byte header.  The copy back leaves the
old header words at frame offset 72, and `sorted_entries` takes those
...
theorem twp_rm_accept_copy [WasmSmallStepGS hlc Universal.State]
  ...
  iapply twp_rm_free_input hfunc2 heapId cap bufPtr (pairCount input)
    ...
    hpayload (acceptedEntries_length input) hcap hspare
  iframe
...
theorem twp_remove_tail [WasmSmallStepGS hlc Universal.State]
  ...
  · -- the accept arm
    ...
    iapply twp_rm_accept_copy hfunc2 heapId cap' buffer oldLen
      ...
      (keyPayload_serialize input haccept).symm hcapBound hspareLen
    iframe
  · isplit
    · -- the reject arm
