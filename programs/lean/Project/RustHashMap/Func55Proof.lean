...
private abbrev func55ArithmeticPrefix : Program :=
...
private abbrev func55GrowthTail : Program :=
...
private abbrev func55InnerBody : Program :=
  func55ArithmeticPrefix ++ func55GrowthTail

private abbrev func55OuterBody : Program :=
  [.block 0 0 func55InnerBody, .call 59, .unreachable]

private abbrev func55CommitTail : Program :=
  [.const 0, .localGet 1, .store32 1049496, .localGet 2]

private abbrev func55Locals
...
private theorem twp_func55_commit_and_return
    ...
    iprop(
      ...
      (∀ resultBytes : List UInt8,
        ...
        ResumeWP [.i32 base] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func55Locals currentPages finish base requiredPages [.i32 0],
          [.localGet 1, .store32 1049496, .localGet 2],
          1, [], [],
  ...
  wasm_twp_bind twp_store32 (address := 0) (offset := 1049496) (value := finish)
...
private theorem twp_func55_claim_commit_and_return
    [WasmSmallStepGS hlc Universal.State]
    (currentPages finish base requiredPages storedCursor : UInt32)
    (layout : AllocLayout) (heapId : GName)
    ...
    iprop(
      ...
      (∀ resultBytes : List UInt8,
        ...
        ResumeWP [.i32 base] callerLocals stack code arity remainder controls
          calls s E Φ)) ⊢
      WP (.running
        ⟨func55Locals currentPages finish base requiredPages,
          func55CommitTail, 1, [], [],
          { locals := { callerLocals with values := stack }
            continuation := code
            resultArity := arity
...
private theorem twp_func55_oom
    [WasmSmallStepGS hlc Universal.State]
    (first finish base requiredPages : UInt32)
    ...
    iprop(
      ...
      WP (.running
        ⟨func55Locals first finish base requiredPages,
          [.call 59, .unreachable], 1, [], functionControls,
          { locals := { callerLocals with values := stack }
            continuation := code
            resultArity := arity
            callerRemainder := remainder
  ...
  have Hoom := Project.RustHashMap.ImportProofs.func56_correct (hlc := hlc)
      ...
      (callerLocals := func55Locals first finish base requiredPages)
...
theorem func55_correct_pow2 [WasmSmallStepGS hlc Universal.State] :
  ...
  wasm_twp_rebind Wasm.SmallStep.twp_call Project.RustHashMap.«module» 58
  ...
  · have hfinishHigh : 2147483648 ≤ finishNat := by omega
    ...
    have Hfailure := twp_func55_oom size finish base
        ...
        [{ kind := .block
           paramArity := 0
           resultArity := 0
           body := func55OuterBody
           continuation := func55CommitTail
           belowStack := [] }]
        controls calls s E Φ
    simp only [func55Locals, func55OuterBody, func55InnerBody,
      func55ArithmeticPrefix, func55GrowthTail, func55CommitTail,
  ...
  · have hfinishSigned : finishNat < 2147483648 := by omega
    ...
    · iapply twp_leU (result := 1) (by rw [if_pos hfits])
      ...
      iclose_map_runtime Hruntime with Hmodule Henv
      iapply twp_func55_claim_commit_and_return pages.toUInt32 finish base
    ...
    · iapply twp_leU (result := 0) (by rw [if_neg hfits])
      ...
      iapply twp_memoryGrow_tracked Project.RustHashMap.«module» ⟨0⟩ pages
          (fun actualPages hmeasured => by
            ...
            have Hfailure := twp_func55_oom pages.toUInt32 finish base
          ...
          (fun oldPages previousPages newPages hfacts hmeasured => by
            ...
            · iapply twp_ne (result := 1) (by simp [hsentinel])
              ...
              ihave Hnormal := BI.and_elim_l $$ Hcont
              iclose_map_runtime Hruntime with Hmodule Henv
              iapply twp_func55_claim_commit_and_return pages.toUInt32 finish
