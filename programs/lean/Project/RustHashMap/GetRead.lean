...
/-!
# The read phase of `map_get`

The `map_get` driver is local `func18`, absolute function 21.  It takes a
...
This file is the instance of the read-loop core at `getMap`, the frame map
of absolute function 21.  The driver declares six locals, so the core gets
a local tail of one slot.  The read phase never writes that slot, so it
holds zero throughout.
...
namespace Project.RustHashMap.GetRead
...
/-! ## The frame of `map_get` -/

/-- The frame base of the `map_get` driver: 320 bytes below the entry
stack top. -/
def func18Base : UInt32 := entryStackTop - 320

/-- The code of the `map_get` driver after the read phase.  The frame
setup is 19 instructions and the read block is the 20th. -/
def func18AfterRead : Program := Project.RustHashMap.func18.drop 20

theorem func18_shape :
    Project.RustHashMap.func18 =
      readPrologue getMap ++
        .block 0 0 (readPhaseBody getMap) :: func18AfterRead := by
  rfl
...
def afterReadLocalsGet (length capacity ptr aux4 : UInt32) : Locals :=
  ⟨[], [.i32 func18Base, .i32 length, .i32 capacity, .i32 ptr,
    .i32 aux4, .i32 0], []⟩

/-- The resources of the driver after the read phase: the whole input is
in the vector, and the input stream is empty. -/
def AfterReadGet [WasmSmallStepGS hlc Universal.State]
  ...
  iprop(
    RuntimeContext ∗
    StackPointer func18Base ∗
    StackReserve (func18Base - 16) reserve ∗
    Slices.ByteSlice 0 func18Base head ∗
    VecU8 heapId (func18Base + 52) capacity ptr input ∗
    Slices.ByteSlice 0 (func18Base + 64) chunk ∗
...
def phaseFrameGet (afterRead : Program) : ControlFrame :=
  { kind := .block, paramArity := 0, resultArity := 0,
    body := .block 0 0 (firstReadBody getMap) :: .const 0 ::
      .localSet 1 :: .loop 0 0 (readLoopBody getMap) :: reloadVec getMap,
    continuation := afterRead, belowStack := [] }

private theorem func18_initialLocals :
    Project.RustHashMap.func18Def.toLocals [] =
      ⟨[], [.i32 0, .i32 0, .i32 0, .i32 0, .i32 0, .i32 0], []⟩ := by
  rfl
...
/-- The read phase of the `map_get` driver: the frame setup, the reads,
and the register reload.  The caller owns the 336 bytes from the reserve
base to the entry stack top. -/
theorem twp_get_read_phase [WasmSmallStepGS hlc Universal.State]
    ...
    iprop(
      ...
      Slices.ByteSlice 0 (func18Base - 16) frameBytes ∗
      BumpHeap heapId storedCursor frontier history ∗
  ...
  wasm_twp_rebind twp_store32 (address := func18Base)
    (offset := getMap.vecOff + 8) oldLength (by decide) (by decide)
    (by decide) (by decide) with HoldLength
  wasm_twp_pures [twp_localGet]
  iapply twp_pureStep _ _ _ (fun _ => Step.constI64)
  wasm_twp_bind twp_store64 (address := func18Base)
    (offset := getMap.vecOff)
    (oldCapacity.toUInt64 ||| (oldPtr.toUInt64 <<< 32))
    (by decide) (by decide) (by decide) (by decide) (by decide) (by decide)
    (by decide) (by decide) with HoldPair => Hpair
  wasm_twp_pures [twp_localGet twp_const twp_add]
  rw [show getMap.chunkOff + func18Base = func18Base + getMap.chunkOff
    by decide]
  ...
  ihave Hchunk : Slices.ByteSlice 0 (func18Base + getMap.chunkOff)
  ...
  ihave HpairWords : iprop(
      pointsTo_u32 0 (func18Base + getMap.vecOff) 0 ∗
      pointsTo_u32 0 (func18Base + getMap.vecOff + 4) 1) $$ [Hpair]
  · iapply (pointsTo_u32_pair_as_u64 (func18Base + getMap.vecOff) 0 1).mpr
  ...
  ihave HoldLength := pointsTo_u32_address_eq
    (vec_addr getMap func18Base 8).symm $$ HoldLength
  ihave Hvec : VecU8 heapId (func18Base + getMap.vecOff) 0 1 [] $$
  ...
  iapply twp_read_chunk getMap func18Base (List.replicate 256 0)
    input output []
    [.i32 func18Base, .i32 0, .i32 0, .i32 0, .i32 0, .i32 0]
  ...
  · -- an empty input: leave the read block at once
    ...
    isimp only [afterReadLocalsGet] at Hcont
    ihave Hnormal := BI.and_elim_l $$ Hcont
    iapply Hnormal $$ %0 %1 %storedCursor %frontier %history
      %(frameBytes.take 16) %head %(List.replicate 256 0) %0
    isimp only [show func18Base + getMap.chunkOff = func18Base + 64
      by decide] at Hchunk
    isimp only [show func18Base + getMap.vecOff = func18Base + 52
      by decide] at Hvec
    unfold AfterReadGet
  ...
  · -- a nonempty input: run the loop
    ...
    have Hloop := twp_read_loop hfunc98 getMap func18Base heapId
      input output [.i32 0] (reloadVec getMap) arity remainder
      (phaseFrameGet afterRead :: controls) calls s E Φ (by decide)
      getMap_wf initial
    simp only [loopLocals, initial, htakeLen, phaseFrameGet, firstReadBody,
    ...
    · iintro %finalCapacity %finalPtr %finalStoredCursor %finalFrontier
      ...
      wasm_twp_pures [twp_localGet]
      wasm_twp_rebind twp_load32 (address := func18Base)
        (offset := getMap.vecOff + 8)
      ...
      ihave Hvec : VecU8 heapId (func18Base + getMap.vecOff)
...
end Project.RustHashMap.GetRead
