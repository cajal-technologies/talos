...
`add` compiles to the frame-less body that is definitionally CodeLib's
`addFunc`, so each `.call` in `add_sum3` reuses `add_wp`. -/
...
`sub` compiles to the frame-less body that is definitionally CodeLib's
`subFunc`, so each `.call` in `sub_chain3` reuses `sub_wp`. -/
...
`mul` compiles to the frame-less body that is definitionally CodeLib's
`mulFunc`, so each `.call` in `mul_prod3` reuses `mul_wp`. -/
...
`div` inlines the guarded `i64.div_u`. Its opt-0 body shares the `divGuard`
block with CodeLib's `divFunc`; only the (unreachable, since the divisors are
nonzero) panic tail differs. `div_wp` is generic in that tail, so each `.call`
in `div_chain` reuses it. -/
...
`tail` of this crate's `div` shim is inferred from its `Function` record. -/
