...
| `func0 -> func8` | `(oldPtr, oldCapacity*elementSize, alignment,newSize)` | result-place ownership; valid live old block; exact/no-wrap old-size product; prefix bound; heap frontier | fresh live block with copied prefix and old block transferred to retired heap, or exact OOM | The compiled `Func8Spec` proof transfers complete blocks and the exact copied prefix; `GrowSourceOwn`, `growCopied`, and `growHistory` reassemble the proved `Func0Spec` result. |
| `func0 -> func4; func5` | marker, then `(newSize,alignment)` when old capacity is zero and size is nonzero | result place and allocator state under separating ownership; valid layout; heap/frontier | marker preserves all state; allocator returns a fresh nonnull live block or exact OOM | The compiled `Func4Spec` and `Func5Spec` proofs provide the exact identity/allocation effects. Frontier authority plus the below-frontier sparse domain proves freshness/disjointness, and reachable arithmetic proves nonnull/alignment in the proved `Func0Spec`. |
| `func1 -> func43` (addition guard) | `(0,0)` iff `length+additional` wraps | exact unsigned no-wrap fact | edge is false | The `Func1Spec` proof derives the branch condition locally from `initialized.length + current.length < UInt32.size`. |
| `func1 -> func0` | `(sp+4,oldCap,oldPtr,newCap,alignment,elementSize)` | owned 16-byte shadow frame below the 272-byte export frame; result at `sp+4`; Vec live block; geometric lineage | preserved initialized bytes and new `(capacity,pointer,block)`, exact shadow `oldShadow.take 4 ++ serialize [0,newPtr,newCap]`, or exact OOM | `StackReserve_split` gives the exact 4+12 shadow split, `VecStorage_as_growSource` gives the callee source witness, `reserveSuccessShadow` reseals the exact frame, and `GeometricVecFacts.reserveSuccess` proves lineage/history progress and rules out success beyond exponent 29.  Their composition in the compiled `Func1Spec` proof closes this edge. |
| `func1 -> func43` (grow tag) | `(result.+4,result.+8)` when tag is one | a valid selected layout proving `func0` cannot normally return a layout error | edge is false | `Func0Spec` has no tag-one normal post under this specialization; the `func1` proof discharges the edge at the load/test after the call. |
...
| `func2 -> func49` (four sites) | failing index, logical length, static source location | main invariant `i<mid`, `j<n`, `k=i+(j-mid)<n`; remainder equations `i=mid => k=j` and `j=n => k+(mid-i)=n` | every edge is false | `WordSlice_get`/`WordSlice_set` and `SortBuffers_copyFocus` expose the exact bound premise.  The compiled body proof discharges each obligation at its originating guard without an excluded callee spec. |
...
| `func3 -> func43` (values null) | `(4,inputByteLength)` | normal `func5` result is nonnull | edge is false | `LiveBlock` plus `classifyBump_success_reachable` supplies nonnull directly; the driver proof discharges the guard at WAT line 774. |
...
| `func3 -> func43` (scratch null) | `(4,4*n)` | normal `func9` result is nonnull | edge is false | `LiveBlock` plus `classifyBump_success_reachable` supplies nonnull directly; the driver proof discharges the guard at WAT line 779. |
