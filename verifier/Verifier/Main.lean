...
private def generatedHeader (c : Crate) : List String :=
...
private def generatedFooter (c : Crate) : List String :=
  ["", s!"end Project.{snakeToPascal c.name}", ""]

private def compactFunctionBindings (watPath : String) (m : Wasm.Module) : String :=
  String.intercalate "\n\n" <| m.funcs.mapIdx fun i function =>
    s!"def func{i} : Wasm.Program := watFunctionBody% {repr watPath} {i}\n\n" ++
      Emit.funcDef i function
...
private def emitCompactProgramFile (c : Crate) (m : Wasm.Module) : IO Unit := do
  ...
  let body :=
    String.intercalate "\n" <| generatedHeader c ++ [
      ...
      s!"  \{ (watModuleMetadata% {repr watPath}) with funcs := {functionDefs} }",
      "",
      Emit.driftCheck watPath
    ] ++ generatedFooter c
  IO.FS.writeFile (c.leanDir / "Program.lean") body

/-- Human-readable literal representation for audits and emitter debugging. -/
private def emitExpandedProgramFile (c : Crate) (m : Wasm.Module) : IO Unit := do
  IO.FS.createDirAll c.leanDir
  let watPath := s!"../rust/build/{c.name}/program.wat"
  let body :=
    String.intercalate "\n" <| generatedHeader c ++ [
...
private def emitOneCrate (projectDir : FilePath) (c : Crate) (forceEmit expanded : Bool) : IO Unit := do
...
private def emitPrograms (projectDir : FilePath) (crates : Array Crate)
    (forceEmit expanded : Bool) : IO Unit := do
  for c in crates do
    emitOneCrate projectDir c forceEmit expanded
...
private def cmdEmit (names : List String) (forceEmit expanded : Bool) : IO Unit := do
  let projectDir ← projectDirFromCwd
  let crates ← discoverSelected projectDir names
  printCrateBanner crates
  emitPrograms projectDir crates forceEmit expanded
...
private def cmdCheckIn (projectDir : FilePath) (names : List String)
    (forceEmit expanded : Bool) (noProve : Bool) : IO Bool := do
  let crates ← discoverSelected projectDir names
  printCrateBanner crates
  buildWasm projectDir crates
  emitPrograms projectDir crates forceEmit expanded
...
private def cmdCheck (names : List String) (forceEmit expanded : Bool) (noProve : Bool) : IO Unit := do
  let projectDir ← projectDirFromCwd
  unless ← cmdCheckIn projectDir names forceEmit expanded noProve do IO.Process.exit 1
...
def runEmit (p : Parsed) : IO UInt32 := do
  cmdEmit (p.variableArgs.map (·.as! String) |>.toList)
    (p.hasFlag "force-emit") (p.hasFlag "expanded")
  pure 0
...
def runCheck (p : Parsed) : IO UInt32 := do
  cmdCheck (p.variableArgs.map (·.as! String) |>.toList)
    (p.hasFlag "force-emit") (p.hasFlag "expanded") (p.hasFlag "no-prove")
  pure 0
...
def emitCmd : Cmd := `[Cli|
  emit VIA runEmit;
  "Transpile program.wat → Program.lean and scaffold Spec.lean for selected crates. [--force-emit] [--expanded]"

  FLAGS:
    "force-emit"; "Re-emit even if wasm is older than Program.lean."
    "expanded";   "Emit the complete human-readable Lean AST instead of the compact WAT-backed form."
...
def checkCmd : Cmd := `[Cli|
  «check» VIA runCheck;
  "build → emit → prove for selected crates (omit names for all). [--force-emit] [--expanded] [--no-prove]"
