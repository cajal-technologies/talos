...
private def goldenModule : String := r#"{
  ...
  memory := some ({ pagesMin := 1, pagesMax := some 4, data := [
    { offset := some 7, bytes := [1, 2], memIdx := 2, offsetType := some (.i64), offsetExprPresent := true, offsetExpr := [
      .globalGet 0
     ] }
   ], is64 := true }),
  extraMemories := [
   { pagesMin := 2, pagesMax := some 8, is64 := true }
  ],
  dataWithoutMemory := true,
  globals := [
   { init := .i64 9, declaredType := some (.i64), isMut := false, sourceInit := some ([
     .constI64 9
    ]), initExpr := [
     .globalGet 0
    ] }
...
private def goldenDriftCheck : String := r#"-- Compile-time fidelity check: errors if WAT and generated Lean AST differ.
#guard_msgs (drop info) in
#eval Wasm.checkWatFidelity "rust/build/demo/program.wat" «module»"#

#guard Verifier.Emit.driftCheck "rust/build/demo/program.wat" =
  goldenDriftCheck
